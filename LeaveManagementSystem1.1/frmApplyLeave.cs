...
namespace LeaveManagementSystem1._1
{
    public partial class frmApplyLeave : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            numtime = int.Parse(dtpTimeTo.Value.ToString("hh")) - int.Parse(dtpTimeFrom.Value.ToString("hh"));
            //MsgBox(numtime)
            numdays = int.Parse(dtpenddate.Value.ToString("dd")) - int.Parse(dtpdatestart.Value.ToString("dd")) ;
             //MsgBox(numdays)
            if( numdays >= 0)
            {
                day = 0.5;
